...
pub fn save_game(ecs : &mut World) {
    ...
    //Saving
    {
        ...
        serialize_individually!(ecs, serializer, data, Position, Renderable, Player, Viewshed, Monster, 
            Name, BlocksTile, CombatStats, SufferDamage, WantsToMelee, Item, Consumable, Ranged, 
            InflictsDamage, AreaOfEffect, Confusion, ProvidesHealing, InBackpack, WantsToPickupItem, 
            WantsToUseItem, WantsToDropItem, SerializationHelper, Equippable
        );
    }
    ecs.delete_entity(savehelper).expect("Crash on gamesave cleanup.")
}
    ...
    let mut deleteme : Option<Entity> = None;
    {
        ...
        for (e,h) in (&entities, &helper).join() {
            ...
            *worldmap = h.map.clone();
            worldmap.tile_content = vec![Vec::new(); (map.width * map.height)as usize];
            deleteme = Some(e);
        }
